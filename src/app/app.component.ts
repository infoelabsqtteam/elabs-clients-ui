  ...
  ngOnInit() {  
    ...
    this.router.events.subscribe(event =>{
      ...
      if (event instanceof NavigationEnd) {
        ...
        if (
          event.id === 1 &&
          event.url === event.urlAfterRedirects && !event.url.startsWith("/download-manual-report") && !event.url.startsWith("/verify") && !event.url.startsWith("/pbl") && !event.url.startsWith("/unsubscribe")
        ) {
